import { NumberMap, TimePeriod } from './Types';
...
export class TimePeriodData {
	...
	constructor(timePeriod: TimePeriod) {
		[this.startHour, this.startMin] = timePeriod.startTime
			.split(':')
			.map((t) => parseInt(t, 10));
...
export default class MinutesCalc {
	...
	addPeriod(tpd: TimePeriodData) {
		 ...
		 */
		while (
			compareTimes(tpd.startHour, tpd.startMin, this.previousEndHour, this.previousEndMin) ===
			-1
		) {
